...
export default defineEventHandler(async (event) => {
  await enforceRateLimit(event, 'napkinbets-upload-proof', 10, 60_000)
  ...
  if (fileInfo.data.length > MAX_FILE_SIZE) {
    throw createError({
      statusCode: 413,
  ...
  // Generate a unique object key in R2
  const extension = contentType.split('/')[1] || 'jpg'
  const objectKey = `settlement-proof/${wagerId}/${settlementId}/${crypto.randomUUID()}.${extension}`
  const proofData = Uint8Array.from(fileInfo.data).buffer

  // Upload to R2 (using default 'BUCKET' binding), extracting ArrayBuffer from Buffer
