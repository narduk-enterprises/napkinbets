      ...
      <template #body>
        <UForm :state="createForm" class="space-y-4" @submit="submitCreate">
            ...
            <UFormField label="Description">
              <UTextarea v-model="createForm.description" placeholder="Description of the bet" class="w-full" :rows="3" />
            </UFormField>
            ...
            <UFormField label="Custom Slug (optional)">
              <UInput v-model="createForm.slug" placeholder="super-bowl-lviii-custom" class="w-full" />
            </UFormField>

            <div class="flex justify-end gap-2 pt-4">
              <UButton color="neutral" variant="ghost" @click="showCreateModal = false">Cancel</UButton>
              <UButton type="submit" color="primary" :loading="actions.activeAction.value === 'admin-wager-create'">
                Create
              </UButton>
            </div>
          </UForm>
      ...
      <template #body>
        <UForm :state="editForm" class="space-y-4" @submit="submitEdit">
            ...
            <UFormField label="Description (Leave blank to keep existing)">
              <UTextarea v-model="editForm.description" placeholder="New description..." class="w-full" :rows="3" />
            </UFormField>
            ...
            <div class="flex justify-end gap-2 pt-4">
              ...
              <UButton type="submit" color="primary" :loading="actions.activeAction.value === `admin-wager-update:${selectedWager?.id}`">
                Save changes
              </UButton>
            </div>
          </UForm>
          ...
          <div class="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-error/10">
            ...
            <div>
              ...
              <p class="text-sm text-muted mt-2">
                Are you sure you want to delete <span class="font-medium text-default">{{ selectedWager?.title }}</span>?
                This action cannot be undone and will remove all associated picks, pots, and participants.
              </p>
            </div>
