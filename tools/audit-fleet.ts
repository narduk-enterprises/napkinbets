...
const TEMPLATE_DIR = join(process.env.HOME || '~', 'new-code/napkinbets')
...
jobs:
  quality:
    uses: narduk-enterprises/napkinbets/.github/workflows/reusable-quality.yml@main
    secrets:
...
function main() {
  ...
  try {
    const gscRaw = execSync(
      'doppler secrets get GSC_SERVICE_ACCOUNT_JSON --project napkinbets --config prd --plain',
      { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] },
    ).trim()
