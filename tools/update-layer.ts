...
function resolveTemplateDir(args: string[]): string {
  ...
  try {
    ...
    if (originUrl.includes('narduk-enterprises/napkinbets')) {
      return ROOT_DIR
    }
  } catch {
    /* fall through */
  ...
  try {
    ...
    if (rootPackage.name === 'napkinbets') {
      return ROOT_DIR
    }
  } catch {
    /* fall through */
  ...
  const defaultTemplateDir = join(process.env.HOME || '', 'new-code', 'napkinbets')
...
if (!existsSync(join(templateDir, 'layers/narduk-nuxt-layer'))) {
  ...
  console.error('Pass --from /path/to/napkinbets to select the source template.')
  process.exit(1)
}
