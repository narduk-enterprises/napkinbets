...
```bash
git clone https://github.com/narduk-enterprises/napkinbets.git my-project
cd my-project
pnpm install
...
| ---------------------- | ---------- | -------------- |
| `napkinbets` | `dev`      | Shared secrets |
| `napkinbets` | `dev_auth` | `example-auth` |

**First-time setup for example-auth:**

```bash
cd apps/example-auth
doppler setup --project napkinbets --config dev_auth --no-interactive
```
